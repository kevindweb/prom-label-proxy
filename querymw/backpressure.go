...
var globalBackpressure internalBackpressure

type internalBackpressure struct {
}
...
var _ ThanosClient = &Backpressure{}

func NewBackpressure(querier ThanosClient, minWindow, maxWindow int, queries []string, monitorURL string) *Backpressure {
	return &Backpressure{
		mu:        sync.Mutex{},
		watermark: minWindow,
		min:       minWindow,
...
func (bp *Backpressure) QueryInstant(ctx context.Context, r InstantRequest) error {
	if err := bp.check(); err != nil {
		return err
	}

	defer bp.release()
	return bp.client.QueryInstant(ctx, r)
}

func (bp *Backpressure) QueryRange(ctx context.Context, r RangeRequest) error {
	if err := bp.check(); err != nil {
		return err
	}

	defer bp.release()
	return bp.client.QueryRange(ctx, r)
}
